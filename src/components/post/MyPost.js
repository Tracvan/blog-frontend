...
function Post() {
    ...
    const handleLike = async (postId) => {
        ...
        try {
            const like = await axios.post(`http://localhost:8080/api/react/like?postId=${postId}&size=${newSize}`, null, config);
            const response = await axios.get(`http://localhost:8080/api/posts/users?page=0&size=${newSize}`, config)
            const data = response.data;
            setPosts(data)
        } catch (error) {
            console.error("Error loading posts:", error);
    ...
    const handleUnlike = async (postId) => {
        ...
        try {
            const like = await axios.delete(`http://localhost:8080/api/react/like?postId=${postId}&size=${newSize}`, config);
            const response = await axios.get(`http://localhost:8080/api/posts/users?page=0&size=${newSize}`, config)
            const data = response.data
